...
God.watch do |w|
  w.name = god_config.process_name

  # Set start command
  w.start = SidekiqCommandBuilder::build_start_command(sidekiq_config)

   # Set stop command
  w.stop = SidekiqCommandBuilder::build_stop_command(sidekiq_config, god_config.stop_timeout)
  w.stop_timeout = god_config.stop_timeout

  w.pid_file = sidekiq_config.pidfile
  ...
  w.dir = sidekiq_config.chdir || Rails.root || Dir.pwd
